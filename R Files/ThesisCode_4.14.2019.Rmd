...
### Number of institutions in each year
...
ggplot(aes(x=year, y=count)) +
  ...
  theme_minimal() + labs(title = "Number of Instiutions by Year")
...
##### Density Plot
```{r General Susbidy Distribution}
ggplot(final_regdata_FTE, aes(x=general_subsidy)) + 
 geom_histogram(aes(y=..density..), colour="black", fill="white")+
 geom_density(alpha=.2, fill="#FF6666") + labs(title = "General Subsidy Distribution", x = "General Subsidy Level", y= "Density")
```


```{r general_subsidy_density, fig.height = 30, fig.width = 18, fig.align = "center"}
...
```{r general_subsidy_summary, results='asis'}
...
```{r cache=TRUE}
myprior <- c(prior(normal(0, 10000), class = "Intercept"), prior(normal(0,5), class = "b"), prior(cauchy(0,1), class = "sigma"), prior(gamma(4,1), class = "nu"))

model1 <- brm(data = final_regdata_FTE, family = student, formula = general_subsidy ~ 1 + investment_return, prior = myprior, chains = 4, cores = 4)
```
...
```{r}
myprior <- c(prior(normal(0, 10000), class = "Intercept"), prior(normal(0,5), class = "b", coef = "investment_return"), prior(cauchy(0,1), class = "sigma"), prior(normal(0,5), class = "b", coef = "non_instructional_exp"), prior(gamma(4,1), class = "nu"))

model2 <- brm(data = final_regdata_FTE, family = student, formula = general_subsidy ~ 1 + investment_return + non_instructional_exp, prior = myprior, chains = 4, cores = 4)
```
...
myprior <- c(prior(normal(0, 10000), class = "Intercept"), prior(normal(0,5), class = "b", coef = "investment_return"), prior(cauchy(0,1), class = "sigma"), prior(normal(0,5), class = "b", coef = "non_instructional_exp"), prior(normal(0,1), class = "b", coef = "year2005"), prior(normal(0,1), class = "b", coef = "year2007"), prior(normal(0,1), class = "b", coef = "year2008"), prior(normal(0,1), class = "b", coef = "year2009"), prior(normal(0,1), class = "b", coef = "year2010"), prior(normal(0,1), class = "b", coef = "year2011"), prior(normal(0,1), class = "b", coef = "year2012"), prior(normal(0,1), class = "b", coef = "year2013"), prior(normal(0,1), class = "b", coef = "year2014"), prior(normal(0,1), class = "b", coef = "year2015"), prior(gamma(4,1), class = "nu"))

model3 <- brm(data = final_regdata_FTE, family = student, formula = general_subsidy ~ 1 + investment_return + non_instructional_exp + year, prior = myprior, chains = 4, cores = 4)
```
...
```{r}
myprior <- c(prior(normal(0, 10000), class = "Intercept"), prior(normal(0,5), class = "b", coef = "investment_return"), prior(cauchy(0,1), class = "sigma"), prior(normal(0,5), class = "b", coef = "non_instructional_exp"), prior(normal(0,1), class = "b", coef = "year2005"), prior(normal(0,1), class = "b", coef = "year2007"), prior(normal(0,1), class = "b", coef = "year2008"), prior(normal(0,1), class = "b", coef = "year2009"), prior(normal(0,1), class = "b", coef = "year2010"), prior(normal(0,1), class = "b", coef = "year2011"), prior(normal(0,1), class = "b", coef = "year2012"), prior(normal(0,1), class = "b", coef = "year2013"), prior(normal(0,1), class = "b", coef = "year2014"), prior(normal(0,1), class = "b", coef = "year2015"), prior(normal(0,5), class = "b", coef = "gifts_total"), prior(normal(0,5), class = "b", coef = "grants_appr"), prior(normal(0,1), class = "b", coef = "net_aux_rev"), prior(normal(0,10), class = "b", coef = "net_hospital_rev"), prior(normal(0,0.5), class = "b", coef = "net_other_rev"), prior(gamma(4,1), class = "nu"))

model4 <- brm(data = final_regdata_FTE, family = student, formula = general_subsidy ~ 1 + investment_return + non_instructional_exp + year + gifts_total + grants_appr + net_aux_rev + net_hospital_rev + net_other_rev, prior = myprior, chains = 4, cores = 4)
```
...
if (any(pareto_k_values(loo_model4) > 0.7)) {
  loo_model4 <- loo(model4, save_psis = TRUE, k_threshold = 0.7)
}
...
### Model 5
```{r}
myprior <- c(prior(normal(0, 10000), class = "Intercept"), prior(normal(0,5), class = "b", coef = "investment_return"), prior(cauchy(0,1), class = "sigma"), prior(normal(0,5), class = "b", coef = "non_instructional_exp"), prior(normal(0,5), class = "b", coef = "gifts_total"), prior(normal(0,5), class = "b", coef = "grants_appr"), prior(normal(0,1), class = "b", coef = "net_aux_rev"), prior(normal(0,10), class = "b", coef = "net_hospital_rev"), prior(normal(0,0.5), class = "b", coef = "net_other_rev"), prior(gamma(4,1), class = "nu"))

model5 <- brm(data = final_regdata_FTE, family = student, formula = general_subsidy ~ 1 + investment_return + non_instructional_exp + gifts_total + grants_appr + net_aux_rev + net_hospital_rev + net_other_rev, prior = myprior, chains = 4, cores = 4)

summary(model5)

loo_model5 <- loo(model5, save_psis = TRUE)
print(loo_model5)


if (any(pareto_k_values(loo_model5) > 0.7)) {
  loo_model5 <- loo(model5, save_psis = TRUE, k_threshold = 0.7)
}

yrep <- posterior_predict(model5)

# requires bayesplot version >= 1.5.0
ppc_loo_pit_overlay(
  y = final_regdata_FTE$general_subsidy,
  yrep = yrep,
  lw = weights(loo_model5$psis_object)
)
...
```{r}
loo_compare(loo_model1, loo_model2, loo_model3, loo_model4, loo_model5)
...
fit_vs <- varsel(model5)

varsel_plot(fit_vs, stats = c('elpd','rmse'))
```


```{r}
cvs <- cv_varsel(model5, method = 'forward', verbose = FALSE)
...
### Vizualising the Submodel

```{r}
mcmc_areas(as.matrix(project(fit_vs, nv =1)),
           pars = c('(Intercept)', names(fit_vs$vind[1]),'sigma'))
```